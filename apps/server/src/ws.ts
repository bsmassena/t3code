        ...
        [WS_METHODS.gitGetWorktreeFileDiff]: (input) =>
          observeRpcEffect(
            ...
            Effect.gen(function* () {
              ...
              const [unstaged, staged] = yield* Effect.all(
                [
                  driver.execute({
                    operation: "GitVcsDriver.worktreeFileDiff.unstaged",
                    cwd: input.cwd,
                    args: ["diff", ...baseArgs],
                  }),
                  driver.execute({
                    operation: "GitVcsDriver.worktreeFileDiff.staged",
                    cwd: input.cwd,
                    args: ["diff", "--cached", ...baseArgs],
                  }),
                ],
              ...
              Effect.mapError(
                (cause) =>
                  new GitCommandError({
                    operation: "GitVcsDriver.worktreeFileDiff",
                    command: "git diff",
                    cwd: input.cwd,
                ...
                SourceControlDiscoveryLayer.layer.pipe(
                  Layer.provide(
                    SourceControlProviderRegistry.layer.pipe(
                      ...
                      Layer.provide(
                        VcsDriverRegistryModule.layer.pipe(Layer.provide(VcsProjectConfig.layer)),
                      ),
                    ),
                  ),
