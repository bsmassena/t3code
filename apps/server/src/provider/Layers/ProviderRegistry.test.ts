      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            assert.strictEqual(codexPersonal?.installed, process.platform === "win32");
            if (process.platform === "win32") {
              assert.match(codexPersonal?.message ?? "", /Codex app-server provider probe failed/);
            } else {
              assert.strictEqual(
      ...
      live("re-probes when settings change the codex binaryPath", () =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            assert.strictEqual(initialCodex?.installed, process.platform === "win32");
            ...
            assert.strictEqual(reprobedCodex?.installed, process.platform === "win32");
          }).pipe(Effect.provide(runtimeServices));
        }),
      );
